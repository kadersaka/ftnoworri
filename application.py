...
## extensions Noworritechnology2020
api = Api(application)
...
def resize_video(path, size, outputname):
    ...
    try:
        ...
        s3 = get_client()
        s3.upload_fileobj(p_file, app.config['AWS_BUCKET'], relative_path)
        presigned_url = s3.generate_presigned_url('get_object', Params = {'Bucket': app.config["AWS_BUCKET"] , 'Key': relative_path})
...
def mark_pdf_bytes(text, pdf_file, main_width, main_heigth):
    ...
    """
     ...
     try:
         relative_path = "secure_images/" + final_image_path
         s3 = get_client()
         s3.upload_fileobj(image_path, app.config['AWS_BUCKET'], relative_path)
         presigned_url = s3.generate_presigned_url('get_object', Params = {'Bucket': app.config["AWS_BUCKET"] , 'Key': relative_path})
         ...
         relative_path = "secure_images_copy/" + final_image_path
         s3.upload_fileobj(in_mem_file, app.config['AWS_BUCKET'], relative_path)
         presigned_url = s3.generate_presigned_url('get_object', Params = {'Bucket': app.config["AWS_BUCKET"] , 'Key': relative_path})
